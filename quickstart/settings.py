...
def prompt_users(options, defaults, sub=""):
    result = {}
    for key, value in options.items():
        ...
        if isinstance(value, str):
            ...
            if is_sub_group(value):
                ...
                if "help" in value:
                    help = value['help']
                if ("force" not in value or value['force'] is False) and sub + key not in result:
                    result[sub + key] = [
                        help,
                        prompt(
                            sub + key,
                ...
                if (sub + key in result and result[sub + key][1] is True) or (
                    ...
                    if "force" in value:
                        del value["force"]
                    result = {**result, **prompt_users(value, defaults, sub + key + '.')}
...
def get_options(file, defaults):
    return prompt_users(load_settings(file)['Options'], defaults)
